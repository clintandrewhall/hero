...
{	
	...
	if(argc > 2)
	{
		...
		GCDAsyncSocket* socket = [[GCDAsyncSocket alloc] initWithDelegate:self userData:[userData longLongValue]];
...
{	
	...
	if(argc > 0)
	{
		...
		{
			GCDAsyncSocket* sock = (GCDAsyncSocket*)[connectedSockets objectAtIndex:x];
 ...
 * If aborted, will result in GCDAsyncSocketCanceledError.
 * 
 ...
 * It may be used to for things such as updating progress bars.
 **/
- (void)onSocket:(GCDAsyncSocket *)sock didWritePartialDataOfLength:(CFIndex)partialLength tag:(long)tag
{
